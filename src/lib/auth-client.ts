...
// No Node.js specific modules like bcrypt, fs, etc.
...
export async function login(email: string, password: string) {
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_DATABASE_WORKER_URL}/auth/login`, {
      method: 'POST',
      headers: {
    ...
    setCookie('auth-token', result.token, 1);
...
export async function signup(name: string, email: string, password: string, plan: string) {
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_DATABASE_WORKER_URL}/users`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        ...
        email,
        name,
        password, // The server will hash the password
        plan,
...
export async function requestPasswordReset(email: string) {
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_DATABASE_WORKER_URL}/auth/forgot-password`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
export async function verifyPasswordResetToken(token: string) {
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_DATABASE_WORKER_URL}/auth/reset-password/verify`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
export async function resetPassword(token: string, newPassword: string) {
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_DATABASE_WORKER_URL}/auth/reset-password`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
